...
int main(int argc, char *argv[]) {
   ...
   char NickName[MAX_CLIENTS*2][30];
   char msg1[100];
   ...
   if(argc==2 || argc==4){
     ...
     printf("###Enter \"set nickname:(your nickname)\" to change you name!!\n");
     fflush(stdout);
     ...
     while (1) {
       ...
       for(fd=0;fd<FD_SETSIZE;fd++){
		...
		if (strncmp(kb_msg, "set nickname:",13)==0){
		   sprintf(msg,"N%s",kb_msg+13);
		   write(sockfd,msg,strlen(msg));
		}
                else {
     ...
     while (1) {
        ...
        for (fd = 0; fd < FD_SETSIZE; fd++) {
           if (FD_ISSET(fd, &testfds)) {
		    ...
		    sprintf(NickName[client_sockfd],"%2d",client_sockfd);
                    /*Client ID*/
		    ...
		    if(msg[0] == 'N'){
		       strcpy(msg1,NickName[fd]);
		       bzero(NickName[fd],30);
		       strncpy(NickName[fd],msg+1,result-2);
		       bzero(msg,MSG_SIZE+1);
		       sprintf(msg," %s change name to %s\n",msg1,NickName[fd]);
		     }
		     else msg[result]='\0';
		     bzero(kb_msg,MSG_SIZE+10);
                    /*read 2 bytes client id*/
                    sprintf(kb_msg,"M%s:",NickName[fd]);                  
